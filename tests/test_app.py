...
event = {
    ...
    "prompt": [
        {
            "type": "point",
            "data": {"lat": 38.48542007717153, "lng": 14.921846904165468},
            "label": 0,
        }
    ],
    "zoom": 10,
...
response_bodies_post_test = {
    "single_point": {
        "bbox": [
            [39.036252959636606, 15.040283203125002],
            [38.302869955150044, 13.634033203125002],
        ],
        "prompt": [{"type": "point", "label": 0, "data": [937, 514]}],
        "zoom": 10,
        "source": source,
    },
    "multi_prompt": {
        "bbox": [
            [39.011714588834074, 15.093841552734377],
            [38.278078995562105, 13.687591552734377],
        ],
        "prompt": [
    ...
    "single_rectangle": {
        ...
        "prompt": [{"type": "rectangle", "data": [875, 445, 951, 538]}],
        "zoom": 10,
        "source": source,
    },
}
...
class TestFastapiApp(unittest.TestCase):
    ...
    def test_infer_samgis_real_200(self, time_mocked, get_url_tile_mocked):
        ...
        local_tile_provider = xyzservices.TileProvider(
            name="local_tile_provider", url=LOCAL_URL_TILE, attribution=""
        )
        get_url_tile_mocked.return_value = local_tile_provider

        with LocalTilesHttpServer.http_server(
            "localhost", listen_port, directory=TEST_EVENTS_FOLDER
        ):
            response = client.post(infer_samgis, json=event)
            logging.info(f"response.status_code: '{response.status_code}'.")
        test_client_health.check_for_statuscode(response.status_code, 200, response)
        ...
        logging.info(f"response.body_loaded: '{body_loaded}'.")
        ...
        logging.info(f"output_geojson:: '{output_geojson}'.")
    ...
    def test_infer_samgis_mocked_200(self, samexporter_predict_mocked, time_mocked):
        ...
        self.assertDictEqual(
            body_loaded, {"duration_run": 0, "output": samexporter_output}
        )
